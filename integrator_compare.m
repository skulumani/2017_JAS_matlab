...
x0 = [0.75;0;0;0.2883]';
t0 = 0;
tf = 2000; % figure out how to dimensionalize time
...
for ii = 1:length(num_steps)
    ...
    % add to plot
    set(0,'CurrentFigure',e_fig)
    plot(t_ode4, abs(E_ode4-E_ode4(1)))
    plot(t_trap, abs(E_trap-E_trap(1)))

    set(0, 'CurrentFigure', h_fig)
    plot(h, mean(abs(E_ode4 - E_ode4(1))), 'ro')
    plot(h, mean(abs(E_trap - E_trap(1))), 'gx')
