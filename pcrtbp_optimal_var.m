...
function [t, state,H] = pcrtbp_optimal_var(x0,h0,t0,tf, mode, num_steps, constants)
...
switch mode
    case 'rect'
        % this uses the sub functions for hte state and costate update
        %         for k = 1:num_steps-1
        %
        ...
        %             end
        %
        %             next_state = pcrtbp_var_rect_update(state(k,:),h,mu,u);
        %
        %             state(k+1,:) = next_state;
        %
        %             jacobian = pcrtbp_var_rect_jacobian(state(k,:),h,mu);
        %
        %             costate(k+1,:) = costate(k,:)/(jacobian);
        %
        %
        %             H(k) = costate(k+1,:)*state(k,:)';
        %         end % end of for loop
    ...
    case 'trap'
        % this loop uses the sub functions which is slower
        %         for k = 1:num_steps-1
        %
        %             switch constants.control_switch
        %                 case 'off'
        ...
        %             end
        %
        %             next_state = pcrtbp_var_trap_update(state(k,:),h,mu,u);
        %
        %             state(k+1,:) = next_state;
        %
        %             jacobian = pcrtbp_var_trap_jacobian(state(k,:),h,mu);
        %
        %             costate(k+1,:) = costate(k,:)/(jacobian);
        %
        %
