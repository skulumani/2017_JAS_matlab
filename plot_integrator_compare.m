function plot_integrator_compare(filename)
    ...
    for ii = 1:length(num_steps)
        ...
        h = (tf - t0) / ns;
        set(0,'CurrentFigure',e_fig)
        plot(t_vec, abs(Ehist_ode45(ii,1:ns)-Ehist_ode45(ii,1)), 'r')
        plot(t_vec, abs(Ehist_ode4(ii,1:ns) - Ehist_ode4(ii,1)), 'b')
        plot(t_vec, abs(Ehist_trap(ii,1:ns)-Ehist_trap(ii,1)), 'g')
